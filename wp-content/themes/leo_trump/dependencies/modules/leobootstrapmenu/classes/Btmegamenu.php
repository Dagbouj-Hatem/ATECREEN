...
class Btmegamenu extends ObjectModel
{
    ...
    public function validateFields($die = true, $error_return = false)
    {
        ...
        if (!$id_btmegamenu = (int)Tools::getValue('id_btmegamenu')) {
            $number_menu = Btmegamenu::freeCountMenu();

            if($number_menu >= LeoBtmegamenuHelper::FREE_LIMIT_MENU){
                $message = LeoBtmegamenuHelper::freeMenu();
    ...
    public static function freeCountMenu($id_lang = null, $id_shop = null)
    {
        if (!$id_shop) {
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'btmegamenu menu';
        $sql .= ' INNER JOIN '._DB_PREFIX_.'btmegamenu_shop menushop ON menushop.id_btmegamenu = menu.id_btmegamenu AND id_shop='.$id_shop;
        $menu = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        $number_menu = count($menu);
        return $number_menu;
    }
}
