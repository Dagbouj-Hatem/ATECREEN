...
class LeoSlideshow extends Module
{
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('importGroup')) {
            $this->html .= $this->displayError( LeoSlideshowHelper::freeTextWhensubmit() );
        }
...
//            $this->context->smarty->assign('errors', $this->errors);
        ...
        if (Tools::isSubmit('editgroup') || Tools::isSubmit('submitGroup') || Tools::isSubmit('deletegroup') || Tools::isSubmit('addNewGroup') || Tools::isSubmit('exportgroup') || Tools::isSubmit('changeGStatus')) {
            ...
            $this->html .= LeoSlideshowHelper::freeText().$this->renderConfig();
            $this->html .= LeoSlideshowHelper::freeText().$this->renderSliderForm();
        } else {
            # Action for slideshow
            $this->html .= $this->renderGroupList();
            $this->html .= LeoSlideshowHelper::freeText().$this->renderGroupConfig();
        }

        return LeoSlideshowHelper::freeText().$this->html.LeoSlideshowHelper::freeText();
    }
...
//            'exportLink' => Context::getContext()->link->getAdminLink('AdminLeoSlideshow').'&exportGroup=1',
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitGroup')) {
            ...
            if (!$id_group = (int)Tools::getValue('id_group')) {
                $number_group = LeoSlideshowGroup::freeCountGroup();

                if($number_group >= LeoSlideshowHelper::FREE_LIMIT_GROUP){
                    $this->html .= $this->displayError(LeoSlideshowHelper::freeGroup());
                    return;
                }
            }
    ...
    public function slideProcessAjax()
    {
        ...
        if (Tools::getValue('action') && Tools::getValue('action') == 'submitslider') {
            ...
            if (!$id_slide = (int)Tools::getValue('id_slide')) {
                $number_slide = LeoSlideshowSlide::freeCountSlide();

                if($number_slide >= LeoSlideshowHelper::FREE_LIMIT_SLIDE){
                    $error = LeoSlideshowHelper::freeMenu();
                    $result = array('error' => 1, 'text' => $error);
                    $this->clearCache();
